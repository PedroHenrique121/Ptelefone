...
const Item = ({ title, telefone, email }) => (
  ...
  return (
    <View style={{ flex: 1, alignItems: 'center', marginTop: 60 }}>
      <FlatList
        data={DATA}
        renderItem={({ item }) => (
          <Pressable onPress={() => navigation.navigate('AlteraContatos', {title: item.title, telefone: item.telefone, email: item.email})}>
            <Item title={item.title} telefone={item.telefone} email={item.email} />
          </Pressable>
        )}
...
function CadastroContato({ navigation }) {
  return (
    <View style={{ flex: 1, alignItems: 'center', marginTop:60}}>
      ...
      <Button style={{ wborderBlockColor: 'black', borderWidth: 1, borderRadius: 5, marginBottom: 10 }}
        title="Salvar"
        onPress={() => navigation.navigate('ListaContatos')}
      />
    </View>
...
function AlteraContatos({ navigation , route}) {
   const { title, telefone, email } = route.params;
    return (
    <View style={{ flex: 1, alignItems: 'center', marginTop:50 }}>
      ...
      <View style={{ width:200, marginTop:30}}>
        <View style={{ wborderBlockColor: 'black', borderWidth: 1, borderRadius: 5, marginBottom: 10 }}>
          <Button
            title="Alterar"
            color={'red'}
        ...
        <View style={{ borderBlockColor: 'black', borderWidth: 1, borderRadius: 5 }}>
          ...
          <Button title="Excluir" onPress={() => navigation.navigate('Cadastro')} />
        </View>
        </View>
    </View>
  );
